...
function walk(dir, limit=100000){
  let out = []
  const items = fs.readdirSync(dir, { withFileTypes: true })
  for(const it of items){
    const p = path.join(dir, it.name)
    if (it.isDirectory()){
      if (['node_modules','.git','.next'].includes(it.name)) continue
      try{ out = out.concat(walk(p)) }catch(e){}
    }
    else out.push(p)
...
const outPath = path.join('backups','found-local-similar-all.csv')
...
for(const s of similar.slice(0,100)) console.log(s.missing,'->',s.local)
