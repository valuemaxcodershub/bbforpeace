// Aggressive rerun using manual reupload CSV as input
// Usage: node scripts/aggressive-rerun-manual.js
...
(async function(){
  ...
  for (const line of lines) {
    ...
    const orig = (parts[4] || parts[3] || '').replace(/"$/,'').replace(/^"/,'');
    console.log('\n[Aggressive-Manual] id', id, '-', filename);
    const variants = generateVariants(orig);
    let success = false;
    for (const v of variants) {
      try {
        console.log('  trying', v);
        const buf = await fetchBuffer(v, process.env.BLOB_READ_WRITE_TOKEN || process.env.BLOB_TOKEN || undefined, 6);
        const key = sanitizeKey(id, filename || `file-${id}`);
        await uploadToR2(key, buf, undefined);
    ...
    if (!success) console.warn('  all attempts failed for', id);
  }
  await client.end();
  console.log('\nAggressive manual rerun complete');
})();
